...
def mesh_clustering(deltas,thsh1,thsh2,num_clusters,neutral):
    '''
    ...
    offset /= offset.max(0)
    vtx_include = np.where(offset.max(1)>thsh1)[0]
    offset = offset[vtx_include]
    offset[offset<thsh2] = 0    
    kmeans = KMeans(n_clusters=num_clusters).fit(offset)
    max_score = silhouette_score(offset,kmeans.labels_)
    vtx_labels = kmeans.labels_
    scores = [max_score]
    for i in range(10):
        kmeans = KMeans(n_clusters=num_clusters).fit(offset)
        ssc = silhouette_score(offset,kmeans.labels_)
        scores.append(ssc)
        if ssc > max_score:
            max_score = ssc
            vtx_labels = kmeans.labels_        
    return vtx_labels,max_score,vtx_include,offset
...
def complete_clustering(deltas,thsh1,thsh2,num_clusters,neutral,m,factor=1,merge=True):
    vtx_labels,_,_,offset = mesh_clustering(deltas,thsh1,thsh2,num_clusters,neutral)
